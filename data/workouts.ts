    ...
    number,
    {
      ...
      exercises: Map<
        string,
        {
          ...
          sets: { setNumber: number; reps: number; weightLbs: string | null }[];
        }
      >;
    }
  >();
  ...
  for (const row of rows) {
    ...
    if (row.exerciseName) {
      if (!workout.exercises.has(row.exerciseName)) {
        workout.exercises.set(row.exerciseName, { muscleGroup: row.muscleGroup, sets: [] });
      }
...
export type WorkoutForDate = Awaited<ReturnType<typeof getWorkoutsForDate>>[number];
